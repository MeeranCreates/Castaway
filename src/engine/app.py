...
class GameApp:
    ...
    def sun_direction(self):
        angle = self.time * 0.12
        return glm.normalize(glm.vec3(math.cos(angle), math.sin(angle), math.sin(angle * 0.75)))

    def sky_colors(self):
        sun = self.sun_direction()
        daylight = max(0.0, min(1.0, (sun.y + 0.2) / 0.9))
        night = glm.vec3(.02, .03, .08)
        dawn = glm.vec3(.88, .58, .48)
        day = glm.vec3(.38, .62, .82)
        zenith = glm.vec3(.03, .12, .27)
    ...
    def render_scene_objects(self, view, projection, light_space):
        sun = self.sun_direction(); ambient = glm.mix(glm.vec3(.02, .03, .08), glm.vec3(.24, .34, .43), max(0.0, min(1.0, (sun.y + 0.2) / 0.9)))
        ...
        for name, value in (("uView", view), ("uProjection", projection), ("uLightSpace", light_space),
                            ...
                            ("uAmbientSky", ambient)): s.set(name, value)
    ...
    def render_world(self, view, projection, light_space):
        ...
        for name, value in (("uModel", glm.mat4(1)), ("uView", view), ("uProjection", projection), ("uLightSpace", light_space),
                            ("uCameraPos", self.camera.position), ("uSunDirection", sun),
                            ("uAmbientSky", ambient), ("uRoughness", .72)): s.set(name, value)
    ...
    def present(self, view):
        ...
        sun = self.sun_direction(); fog = glm.mix(glm.vec3(.02, .03, .08), glm.vec3(.52, .63, .70), max(0.0, min(1.0, (sun.y + 0.2) / 0.9)))
        ...
        self.post_shader.set("uFogColor", fog); self.screen.draw()
