...
void main(){
    ...
    vec3 p=aPosition;
    p.x+=sin(uTime*2.+model[3].x*.21+model[3].z*.13)*p.y*.16;
    vec4 w=model*vec4(p,1.);
    vWorldPos = w.xyz;
    vLight=.55+.45*max(dot(vec3(0,1,0),normalize(vec3(.45,.75,-.35))),0.);
    vVariation=fract(sin(dot(model[3].xz,vec2(12.9898,78.233)))*43758.5453);
